...
{ 
	...
	std::condition_variable *GunVar;
	int *readycount;
	std::condition_variable *countvar;
	bool* readycheck;
	...
	{ 
		...
		switch (currentGame->currentGameState)
		{
			...
			case GameState::Won:
				...
				///////////////////////////////////////////////////////////////////////////////////
				currentGame->currentGameState = GameState::Won;
				currentGame->gameCondition.notify_one();
				return;
			case GameState::Draw:
				///////////////////////////////////////////////////////////////////////////////////
				...
				//   out of the PlayGame function
				///////////////////////////////////////////////////////////////////////////////////
				currentGame->currentGameState = GameState::Draw;
				currentGame->gameCondition.notify_one();
				return;
		}
	}
...
void PlayerThreadEntrypoint(Player *currentPlayer)
{
	...
	//
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> gn(*currentPlayer->playerPool->GunMutex);
	(*currentPlayer->playerPool->CountMutex).lock();
	(*currentPlayer->playerPool->readycount)++;
	(*currentPlayer->playerPool->countvar).notify_all();
	(*currentPlayer->playerPool->CountMutex).unlock();
	(*currentPlayer->playerPool->GunVar).wait(gn, [&]() {return *currentPlayer->playerPool->readycheck == true; });
	gn.unlock();
	...
	///////////////////////////////////////////////////////////////////////////////////
	(*currentPlayer->playerPool->CountMutex).lock();
	(*currentPlayer->playerPool->threadscompleted)++;
	(*currentPlayer->playerPool->GunVar).notify_all();
	(*currentPlayer->playerPool->CountMutex).unlock();
}
...
int main(int argc, char **argv)
{
	...
	std::mutex cmut;
	poolOfPlayers.CountMutex = &cmut;
	std::mutex gmut;
	poolOfPlayers.GunMutex = &gmut;
	std::condition_variable gunvar;
	std::condition_variable countvar;
	poolOfPlayers.GunVar = &gunvar;
	poolOfPlayers.countvar = &countvar;
	int readycount = 0;
	poolOfPlayers.readycount = &readycount;
	bool readycheck = false;
	poolOfPlayers.readycheck = &readycheck;
	int threadscompleted = 0;
	poolOfPlayers.threadscompleted = &threadscompleted;
	...
	///////////////////////////////////////////////////////////////////////////////////
	countvar.wait(Countunilok, [&]() {return totalPlayerCount == readycount; });
	...
	readycheck = true;
	poolOfPlayers.GunVar->notify_all();
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::mutex waiting;
	std::unique_lock<std::mutex> uniwait(waiting);
	(*poolOfPlayers.GunVar).wait(uniwait, [&]() {return threadscompleted == totalPlayerCount; });
